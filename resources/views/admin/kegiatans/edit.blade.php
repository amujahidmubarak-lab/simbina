<x-app-layout>
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-slate-900">Edit Kegiatan: {{ $kegiatan->judul }}</h1>
    </div>

    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden max-w-3xl">
        <form action="{{ route('admin.kegiatans.update', $kegiatan) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="judul" class="block text-sm font-medium text-slate-700 mb-1">Judul Kegiatan <span class="text-red-500">*</span></label>
                <input type="text" name="judul" id="judul" value="{{ old('judul', $kegiatan->judul) }}" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm sm:text-sm" required>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="datetime-local" name="tanggal_waktu" id="tanggal_waktu" value="{{ old('tanggal_waktu', $kegiatan->tanggal_waktu->format('Y-m-d\TH:i')) }}" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm sm:text-sm" required>
                ...
                <div>
                    ...
                    <input type="text" name="tempat" id="tempat" value="{{ old('tempat', $kegiatan->tempat) }}" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm sm:text-sm" required>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div>
                    ...
                    <select name="tipe" id="tipe" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm sm:text-sm" required>
                        <option value="rutin" {{ old('tipe', $kegiatan->tipe) == 'rutin' ? 'selected' : '' }}>Rutin</option>
                        <option value="insidental" {{ old('tipe', $kegiatan->tipe) == 'insidental' ? 'selected' : '' }}>Insidental</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="status" id="status" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm sm:text-sm" required>
                        ...
                        <option value="cancelled" {{ old('status', $kegiatan->status) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="kontrakan_id" id="kontrakan_id" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm sm:text-sm">
                        ...
                        @foreach($kontrakans as $k)
                            <option value="{{ $k->id }}" {{ old('kontrakan_id', $kegiatan->kontrakan_id) == $k->id ? 'selected' : '' }}>{{ $k->nama_kontrakan }}</option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="3" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm sm:text-sm">{{ old('deskripsi', $kegiatan->deskripsi) }}</textarea>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
                ...
                <button type="submit" class="px-4 py-2 bg-primary-600 text-white rounded-lg text-sm font-medium hover:bg-primary-700 transition-colors shadow-sm shadow-primary-600/20">Simpan Perubahan</button>
            </div>
        </form>
    </div>
</x-app-layout>
