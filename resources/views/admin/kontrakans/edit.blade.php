<x-app-layout>
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-slate-900">Edit Kontrakan: {{ $kontrakan->nama_kontrakan }}</h1>
    </div>

    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden max-w-3xl">
        <form action="{{ route('admin.kontrakans.update', $kontrakan) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="nama_kontrakan" id="nama_kontrakan" value="{{ old('nama_kontrakan', $kontrakan->nama_kontrakan) }}" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm sm:text-sm" required>
            ...
            <div>
                ...
                <textarea name="alamat" id="alamat" rows="3" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm sm:text-sm">{{ old('alamat', $kontrakan->alamat) }}</textarea>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="kapasitas" id="kapasitas" value="{{ old('kapasitas', $kontrakan->kapasitas) }}" min="1" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm sm:text-sm" required>
                ...
                <div>
                    ...
                    <select name="status" id="status" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm sm:text-sm" required>
                        <option value="active" {{ old('status', $kontrakan->status) == 'active' ? 'selected' : '' }}>Aktif</option>
                        <option value="inactive" {{ old('status', $kontrakan->status) == 'inactive' ? 'selected' : '' }}>Nonaktif</option>
                    </select>
            ...
            <div>
                ...
                <select name="penanggung_jawab" id="penanggung_jawab" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm sm:text-sm">
                    ...
                    @foreach($users as $u)
                        <option value="{{ $u->id }}" {{ old('penanggung_jawab', $kontrakan->penanggung_jawab) == $u->id ? 'selected' : '' }}>{{ $u->name }} ({{ $u->email }})</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="2" class="w-full rounded-lg border-slate-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm sm:text-sm">{{ old('deskripsi', $kontrakan->deskripsi) }}</textarea>
